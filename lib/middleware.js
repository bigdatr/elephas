...
module.exports = function(done, app, options) {
    ...
    var _publicKeyPinsOptions = middlewareOptions('publicKeyPins', options);
    if ( _publicKeyPinsOptions.sha256s !== null){
        app.use(helmet.publicKeyPins(middlewareOptions('publicKeyPins', options)));
    } else {
        logger.warning('publicKeyPins.sha256s should be set to the sha256s of your public key and backup public key');
    }
